  ...
  const weekBoards = visibleWeeks
    .map((g) => {
      ...
      let entries = Object.entries(players).map(([name, days]) => {
        ...
        const totalTime = times.length ? times.reduce((sum, t) => sum + t, 0) : null;
        return { name, total, days: dayValues.length, bestTime, totalTime };
      });

      const hasTime = entries.some((e) => e.bestTime !== null);
      const rankByPlays = g.rankBy === 'plays';
      if (rankByPlays) {
        // Most days played leads; among players tied on days played, the
        // one with the lower COMBINED time across all those days wins --
        // not just whoever had one fast day. No time recorded sorts last.
        entries.sort((a, b) => {
      ...
      return { ...g, entries, hasTime, rankByPlays };
    })
          ...
          {weekBoards.map((w) => (
            <div key={w.week} className="mb-6">
              <div className="font-bold text-sm mb-2 flex items-center gap-2">
                ...
                {w.rankByPlays && <span className="inline-flex items-center gap-1 text-[10px] font-black uppercase tracking-widest text-accent bg-accent/10 px-2 py-0.5 rounded-full"><Repeat className="w-3 h-3" /> Most plays leads</span>}
                {!w.rankByPlays && w.hasTime && <span className="inline-flex items-center gap-1 text-[10px] font-black uppercase tracking-widest text-accent bg-accent/10 px-2 py-0.5 rounded-full"><Timer className="w-3 h-3" /> Fastest wins</span>}
              </div>
              {w.entries.length === 0 ? (
                ...
                <table className="w-full text-sm">
                  <thead>
                    <tr className="text-zinc-500 text-[10px] uppercase tracking-widest text-left border-b border-white/5">
                      <th className="py-2 w-10"></th>
                      <th className="py-2">Tutor</th>
                      {w.hasTime && <th className="py-2 text-right">{w.rankByPlays ? 'Total time' : 'Best time'}</th>}
                  ...
                  <tbody>
                    {w.entries.map((e, i) => (
                      <tr key={e.name} className={`border-b border-white/5 ${e.name === player ? 'bg-accent/5' : ''}`}>
                        ...
                        {w.hasTime && (
                          <td className={`py-2 text-right font-mono ${w.rankByPlays ? 'text-zinc-500' : 'text-accent'}`}>
                            {w.rankByPlays
                              ? (e.totalTime !== null ? formatTime(e.totalTime) : '—')
                              : (e.bestTime !== null ? formatTime(e.bestTime) : '—')}
                          </td>
                        )}
                        <td className={`py-2 text-right font-mono ${w.rankByPlays ? 'text-accent font-bold' : 'text-zinc-500'}`}>{e.days}</td>
